...
#+TITLE: MineMacs
#+auto_tangle: t
#+property: header-args :noweb yes :eval never-export :mkdirp yes :results silent
...
*** Org-tick
#+begin_src elisp
  (use-package org-tick
    :quelpa (org-tick
             :fetcher github
             :repo "ohm-en/org-ticket") ;; my own package
    :bind ("C-x j" . org-tick-ticket-choice-map))
#+end_src
  ...
  (use-package org
   ...
   (org-todo-keywords
    '((sequence "TODO" "WAITING" "|" "DONE" "CANCELLED")
      (sequence "COMMIT" "|" "COMMITTED")))
   (org-startup-indented t)
   (org-log-into-drawer t)
   :config
   <<my-org-utility-functions>>
...
***** Utility Functions
:PROPERTIES:
:header-args: :noweb-ref my-org-utility-functions
:END:

#+begin_src elisp
  (defun my/org-babel-lob-ingest-current-file ()
    "Add all named source blocks defined in current file to ‘org-babel-library-of-babel’."
    (interactive)
    (if-let ((file (buffer-file-name)))
        (org-babel-lob-ingest file)
      (message "Buffer is not a file. Cannot ingest")))
#+end_src
...
#+begin_src elisp :noweb yes :tangle no
  (defvar-keymap my-org-node-prefix-map
    ...
    "u" #'my/org-node-get-id-as-link
    "y" #'my/org-node-copy-node-as-markdown-link
    "f" #'org-node-find
    "c" #'org-capture ;; TODO: Seperate map and define in org-mode use-package
    "p" #'org-node-hierarchy-set-parent)

  (define-key global-map (kbd "C-c n") my-org-node-prefix-map)
#+end_src

***** Utility Functions
replace org-node-get-title
...
#+begin_src elisp :noweb rw :tangle no
  ...
  (defun my/org-node-get-parent-node (node)
    (cl-assert (org-mem-entry-p node))
    (let ((parent_link (org-mem-entry-property "PARENT" node)))
      (if (stringp parent_link)
          (org-mem-entry-by-id (my/extract-org-id-from-link parent_link))
        nil)))

  (defun my/org-node-get-node-parent-hierarchy (node &optional lower-nodes)
  ...
  LOWER-NODES allow prepending already calculated children and is used internal to keep state."
    (let ((nodes (cons node (or lower-nodes '()))))
      (or (when-let ((parent (my/org-node-get-parent-node node)))
            (my/org-node-get-node-parent-hierarchy parent nodes))
          nodes)))

  (defun my/org-node-get-full-title (node)
    "Returns the title of the NODE prepended with its ancestor node titles."
    (cl-assert (org-mem-entry-p node))
    (mapconcat #'org-mem-entry-title (my/org-node-get-node-parent-hierarchy node) ":"))

  (defun my/org-node-get-alias (node)
    "Returns the alias (property) of the NODE."
    (cl-assert (org-mem-entry-p node))
    (org-mem-entry-property "ALIAS" node))

  (defun my/org-node-get-shorthand-title (node &optional accumulated-title)
    "Returns the title of the NODE prepended with its ancestor node titles.
  Short-circuits if an alias is found, returning the alias and the titles leading up to it."
    (let ((title (if-let* ((alias (my/org-node-get-alias node)))
                     alias
                   (let ((node-title (org-mem-entry-title node)))
                     (or (when-let ((parent (my/org-node-get-parent-node node)))
                           (my/org-node-get-shorthand-title parent node-title))
                         node-title))))
  ...
  (defun my/org-node-get-parent-title-prefix (node)
    "Returns a NODE's hierarchical prefix."
    (when-let ((nodes (butlast (my/org-node-get-node-parent-hierarchy node))))
      (concat (mapconcat #'org-mem-entry-title nodes ":") ":")))
  ...
  (defun my/org-node-get-node-markdown-link (node)
    ...
    (cl-assert (org-mem-entry-p node))
      (my/format-markdown-link
       (my/org-node-get-shorthand-title node) "emacs" (org-mem-entry-id node)))

  ;; TODO: Considerm oving
  ...
  (defun my/org-node-title-affixation-fn (node title)
    (if node
        (list (my/org-node-get-full-title node) "" "@")
      (list title "" "")))

  (defun my/org-node-nodes-equal-p (node1 node2)
    "Check if two org-roam nodes NODE1 and NODE2 are equal."
    (string= (org-mem-entry-id node1)
             (org-mem-entry-id node2)))
  ...
  (defun my/is-child-node-p (possible-child-node parent-node)
    ...
    (if (my/org-node-nodes-equal-p possible-child-node parent-node)
        t
      (let ((direct-parent (my/org-node-get-parent-node possible-child-node)))
        (when direct-parent
          (if (my/org-node-nodes-equal-p direct-parent parent-node)
              t
            (my/is-child-node-p direct-parent parent-node))))))

  ;; TODO: Rename
  (defun my/is-child-id-p (possible-child-id parent-id)
    (let ((possible-child-node (org-mem-entry-by-id possible-child-id))
          (parent-node (org-mem-entry-by-id parent-id)))
...
#+begin_src elisp
  (defun my/org-node-set-parent ()
    ...
    (let* ((node (org-node-read))
           (node-id (org-mem-entry-id node))
           (node-title (org-mem-entry-title node))
           (link (format "[[id:%s][%s]]" node-id node-title)))
      (org-entry-put (point) "PARENT" link)))

  (defun my/org-node-get-id-as-link ()
      "Retrieves the org-id of the current headline or note, formats it as a URI,
      and copies it to the kll-ring."
      (interactive)
  ...
  (defun my/org-node-copy-node-as-markdown-link ()
       ...
       (interactive)
       (if-let* ((node (org-node-read))
                 (node-link (my/org-node-get-node-markdown-link node)))
           (org-kill-new node-link)
         (message "No such node to copy")))
#+end_src

***** Org Node Hooks
#+NAME: org-node-hooks
#+begin_src elisp
  ;; Ref: https://github.com/meedstrom/org-mem/blob/main/org-mem.el
  (defun my/org-node-parent-replace-with-full-path (entry)
    (let* ((title (org-mem-entry-title entry))
           (full-title (concat (my/org-node-get-full-title entry) "@"))
           (existing-hash-value (gethash title org-node--candidate<>entry)))
      (when existing-hash-value
        (remhash title org-node--candidate<>entry))
      (puthash full-title entry org-node--candidate<>entry)))

  (defun my/org-node-parent-hook (parse-results)
    (seq-let (bad-paths file-data entries links problems) parse-results
      (dolist (entry entries)
        (when (and (org-mem-entry-p entry)
                   (org-mem-entry-id entry))
          (my/org-node-parent-replace-with-full-path entry)))))

  (define-minor-mode my/org-node-parent-mode
  ...
  rather than twice."
    ...
    (cond
     (org-node-cache-mode
      (add-hook 'org-mem-post-full-scan-functions #'my/org-node-parent-hook)
      (add-hook 'org-mem-post-targeted-scan-functions #'my/org-node-parent-hook))
     (t
      (remove-hook 'org-mem-post-full-scan-functions #'my/org-node-parent-hook)
      (remove-hook 'org-mem-post-targeted-scan-functions #'my/org-node-parent-hook))))
#+end_src
...
#+end_src
***** Export By Date
#+begin_src elisp :noweb-ref my-org-utility-functions
  (defun my/agenda-for-date (date-string)
    "Return the org-agenda for DATE-STRING (format: YYYY-MM-DD) as a string.
  Example: (my/agenda-for-date \"2026-05-07\")"
    (message date-string)
    (let* ((date (org-read-date nil t date-string))
           (org-agenda-span 'day)
           (org-agenda-start-day (format-time-string "%Y-%m-%d" date)))
      (save-window-excursion
        (org-agenda-list 1 org-agenda-start-day org-agenda-span)
...
#+begin_src elisp
  (use-package eglot-python-preset
    ...
    :after eglot
    :custom
    (eglot-python-preset-lsp-server 'basedpyright) ;; or 'ty
    :config
...
#+begin_src elisp
  (use-package avy
    ;; backward-char (C-b) is not used
    :bind ("C-b" . #'avy-pop-mark) ;; I didn't see a similar command flash.el. This works with it anyways.
    :config
...
*** clipetty
A minor mode to handle copy/paste with the system clipboard.
...
#+begin_src elisp
  (use-package magit
    ...
    :bind (:map magit-status-mode-map
                ...
                ("v" . meow-line))
    :config
    ;; Source: https://github.com/magit/magit/discussions/4657#discussioncomment-2539675
